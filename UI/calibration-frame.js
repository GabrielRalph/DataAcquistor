...
class CalibrationFrame extends HideShow {
  ...
  async calibrate(params = defaultCalibration) {
    if (params.scan.on) await this.calibrateScanningGrid(params.scan.size, params.scan.time);
    if (params.grid.on) await this.calibrateAtPointGrid(params.grid.size, params.grid.time);
  }

  async calibrateScanningGrid(grid = 3, speed = 5){
    let {pointer} = this;
    this.ctype = "m" + grid + "t" + speed;
		...
		for (let [pa1, pa2, pb1, pb2] of ext) {
			let pairs = linspace(1, 0, grid).map(t =>
			...
			for (let [left, right] of pairs) {
				...
				await pointer.show();
				this.recording = true;
				await pointer.moveTo(right, speed * 1000);
  ...
  async calibrateAtPointGrid(grid = 3, counts = 4) {
    let {tl, tr, bl, br} = this;
    this.ctype = "G" + grid + "t" + counts;
...
export {CalibrationFrame, HideShow, SvgResize, dotGrid, defaultCalibration}
